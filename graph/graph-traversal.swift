...
class Graph {
    ...
    func dfs(_ startNode: Node) -> [Int] {
        return dfsHelper(startNode, visited: [startNode.value!])
    }
    ...
    func bfs(_ startNode: Node) -> [Int] {
        ...
        while !toVisit.isEmpty {
            ...
            for edge in node.edges {
                let toNode = edge.nodeTo!
                if !toNode.visited {
                    toNode.visited = true
                    visited.append(toNode.value!)
    ...
    func dfsHelper(_ current: Node, visited: [Int]) -> [Int] {
        ...
        for edge in current.edges {
            if !result.contains(edge.nodeTo!.value!) {
                result.append(edge.nodeTo!.value!)
                result = dfsHelper(edge.nodeTo!, visited: result)
            }
        }
